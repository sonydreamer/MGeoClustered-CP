...
# sony method
def compute_coverage_sony(all_y_test, test_intervals, threshold,):
    """ Compute average coverage and length, and print results
    ...
    if len(all_y_test) != len(test_intervals):
        raise ValueError("The length of all_y_test and test_intervals must be the same.")

    coverage_result = 0
    for i in range(len(all_y_test)):
        gt, prediction_left, prediction_right = all_y_test[i, ...], test_intervals[i, :, 0], test_intervals[i, :, 1]
        in_the_range = np.sum(np.all((gt >= prediction_left) & (gt <= prediction_right)))
        if in_the_range > threshold:
            coverage_result += 1

    coverage = coverage_result / len(all_y_test) * 100
    prediction_left, prediction_right = test_intervals[..., 0], test_intervals[..., 1]
    avg_length = np.mean(prediction_right - prediction_left)
